...
export function createFileKv(filePath, opts) {
...
function createKv(filePath, opts) {
    ...
    try {
        if (existsSync(filePath)) {
            ...
            for (const [k, v] of Object.entries(raw)) {
                if (v && typeof v === "object" && (v.expiresAt === 0 || v.expiresAt > now))
                    map.set(k, v);
            }
        }
    }
    ...
    const alive = (e) => !!e && (e.expiresAt === 0 || e.expiresAt > Date.now());
    return {
        async get(key) {
            const e = map.get(key);
            if (!alive(e)) {
