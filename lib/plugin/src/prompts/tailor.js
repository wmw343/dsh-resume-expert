import { FACTS_SCHEMA_DOC, HONESTY, SAFETY_BOUNDARY, OUTPUT_DISCIPLINE, systemMessage, userMessage, wrap, } from "./common.js";
/**
 * 「按 JD 定制」能力 —— 迁移自 offer-ai 已验证的简历改写提示词。
 * 保留了原有的三段式：前置判断 / 任务 / 安全边界。
 */
export const OFFTOPIC_FLAG = "[OFFTOPIC]";
export const REFUSAL_TAILOR = "这个功能只能用来根据岗位 JD 改写简历，帮不上别的忙。粘贴一段真实招聘 JD 和你的简历试试？";
...
${HONESTY}`;
export function buildTailorMessages(jd, resume, facts) {
    ...
    if (facts) {
        parts.push("", wrap("FACTS", JSON.stringify(facts)), "", "FACTS 是结构化事实（权威来源），RESUME 是当前文字稿（参考表达）。", "请以 FACTS 为准重写，不要凭空新增任何一条事实。");
    }
