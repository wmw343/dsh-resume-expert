import { ARCHETYPE_ORDER, DENSITY_ORDER, DEFAULT_STYLE, MAX_METRICS, normalizeSkills, } from "./model.js";
/**
 * 样式白名单校验。
 *
...
export function normalizeStyle(input) {
    const merged = { ...DEFAULT_STYLE, ...(input ?? {}) };
    if (!DENSITY_ORDER.includes(merged.density))
        merged.density = DEFAULT_STYLE.density;
    if (merged.font !== "sans" && merged.font !== "serif")
        merged.font = DEFAULT_STYLE.font;
    ...
    if (!(merged.archetype in ARCHETYPE_ORDER))
        merged.archetype = DEFAULT_STYLE.archetype;
    return merged;
}
/**
 * 简历 HTML 渲染。
...
export function renderResumeHtml(facts, opts) {
    ...
    const edu = (facts.education ?? [])
      ...
      <div class="item-head">
        ...
        <span class="when">${joinInline([e.degree ?? "", e.major ?? ""])}${has(e.period) ? `　${mdInline(e.period)}` : ""}</span>
      </div>
