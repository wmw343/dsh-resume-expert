...
import { emptyFacts, mergeFacts } from "../model.js";
import { buildIntakeMessages } from "../prompts/intake.js";
import { MAX_INPUT, buildDraft, callLlmJson, looksLikeOverride, newSessionId, sanitizeText, saveDraft, } from "./shared.js";
/**
 * 阶段一：首轮诊断与文字稿。
 *
 ...
 */
export function createIntakeCapability(host) {
    return {
        id: "intake",
        async run(input, ctx) {
            const raw = sanitizeText(input?.raw, MAX_INPUT);
            if (!raw)
                throw new PluginError("E_INPUT_INVALID", "输入为空");
            if (looksLikeOverride(raw))
                throw new PluginError("E_INPUT_INVALID", "输入包含疑似指令内容");
