import { PluginError, ERROR_SPECS, toPluginError } from "../contract/errors.js";
import { extractJson } from "../prompts/common.js";
import { deriveMissing, normalizeSkills, renderMarkdown, scoreCompleteness, scoreModules, stripPlaceholders, } from "../model.js";
...
const OVERRIDE_RE = /(忽略|无视|覆盖|忘记)[\s\S]{0,8}(以上|上面|之前|所有|全部)[\s\S]{0,8}(指令|规则|提示|设定|系统)|ignore[\s\S]{0,40}(previous|above)[\s\S]{0,40}(instruction|prompt|rule)|system[\s\S]{0,3}prompt|developer[\s\S]{0,3}mode|\bDAN\b/i;
/** 剥离控制符与分隔符，防止用户提前闭合 <<< >>> 逃逸出数据区 */
export function sanitizeText(input, maxLen = MAX_INPUT) {
 ...
 */
export async function loadDraft(host, ctx, sessionId) {
    const draft = await host.storage.get(sessionKey(ctx, sessionId));
    if (!draft)
        throw new PluginError("E_SESSION_NOT_FOUND", sessionId);
    if (draft.facts)
        draft.facts.skills = normalizeSkills(draft.facts.skills);
    return draft;
}
export async function saveDraft(host, ctx, draft) {
    await host.storage.set(sessionKey(ctx, draft.sessionId), draft, SESSION_TTL_MS);
}
function coercePriority(v) {
    return v === "high" || v === "medium" || v === "low" ? v : "medium";
}
function coerceMissing(raw) {
    ...
    return raw
        ...
        return {
            ...
            module: (sanitizeText(o.module, 20) || "extras"),
            label,
 ...
 */
export function coerceFacts(raw) {
...
function readErrorCode(e) {
    const c = e?.code;
    return typeof c === "string" && c in ERROR_SPECS ? c : null;
}
/**
 ...
 * 所以先剥掉固定话术前缀与包裹的括号。
 */
function detailWithoutSpec(e, code) {
...
export function mapLlmError(e) {
    ...
    if (code)
        return new PluginError(code, detailWithoutSpec(e, code));
    // ② 文案兜底（老宿主 / 第三方宿主可能只抛裸 Error）
    const msg = e instanceof Error ? e.message : String(e);
    if (/abort|timeout|超时/i.test(msg))
        return new PluginError("E_LLM_TIMEOUT", msg);
    if (/429|限流/.test(msg))
        return new PluginError("E_LLM_BUSY", msg);
    ...
    if (/空内容|非 JSON|格式|JSON/.test(msg))
        return new PluginError("E_LLM_BAD_RESPONSE", msg);
...
async function callLlmJsonOnce(host, messages, ctx, budget) {
 ...
 * 这种最需要它的场景恰好超时。
 */
export async function callLlmJson(host, messages, ctx, budget) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        try {
            ...
            if (!canRetry)
                throw lastError;
            host.logger.warn(`[resume-expert] 模型返回不可用，重试第 ${attempt + 2} 次：${lastError.message}`);
