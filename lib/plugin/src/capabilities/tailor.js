...
import { OFFTOPIC_FLAG, REFUSAL_TAILOR, buildTailorMessages } from "../prompts/tailor.js";
import { extractJson } from "../prompts/common.js";
import { MAX_INPUT, coerceFacts, loadDraft, looksLikeOverride, mapLlmError, sanitizeText, saveDraft } from "./shared.js";
/**
 ...
 */
export function createTailorCapability(host) {
    return {
        id: "tailor",
        async run(input, ctx) {
            const jd = sanitizeText(input?.jd, MAX_INPUT);
            if (!jd)
                throw new PluginError("E_INPUT_INVALID", "需要提供岗位 JD");
