/**
 * DSH 适配层 · LLM 客户端。
 * 从简历专家 host/services.ts 复制的 createRealLlm——OpenAI 兼容协议，
...
export interface LlmConfig {
  ...
  maxTokens?: number;
  /** SenseNova 系参数；DeepSeek 不用，保留字段以维持与原实现一致 */
  ...
  return {
    async complete(messages: ChatMessage[], opts?: LlmCallOptions): Promise<string> {
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        if (resp.status === 429) {
          if (attempt < retries) {
            ...
            if (Number.isFinite(ra) && ra > 90) {
              throw new Error(
                `429 模型服务被限流：服务端要求等待 ${Math.round(ra)}s（通常是用量配额已达上限，需等窗口重置）`
              );
            }
        ...
        if (!resp.ok) throw new Error(`模型服务返回 HTTP ${resp.status}`);
