...
import type { PluginInstance } from "../contract/plugin.ts";
import type { ResumeDraft } from "../model.ts";
import { mdToHtml } from "./md.ts";
import type { ExportOutput } from "../capabilities/export.ts";
...
export function mount(el: HTMLElement, deps: WidgetDeps): () => void {
  ...
  const must = <T extends Element>(sel: string): T => {
    ...
    if (!node) throw new Error(`[resume-expert] 缺少必需的 DOM 节点：${sel}`);
    return node as unknown as T;
  };
  const $ = must;

  const msgsEl = $<HTMLElement>(".rx-msgs");
  const chipsEl = $<HTMLElement>(".rx-chips");
  const missingEl = $<HTMLElement>(".rx-missing");
  const docEl = $<HTMLElement>(".rx-doc");
  const formEl = $<HTMLFormElement>(".rx-form");
  const textareaEl = $<HTMLTextAreaElement>(".rx-form textarea");
  const hintEl = $<HTMLElement>(".rx-hint");
  const barEl = $<HTMLElement>(".rx-bar i");
  const pctEl = $<HTMLElement>(".rx-pct");
  const photoBarEl = $<HTMLElement>(".rx-photo-bar");
  const fileEl = $<HTMLInputElement>('input[type="file"]');
  const composeBtn = $<HTMLButtonElement>('[data-act="compose"]');
  const exportBtn = $<HTMLButtonElement>('[data-act="export"]');
  const sendBtn = $<HTMLButtonElement>('.rx-form button[type="submit"]');
  const pickPhotoBtn = $<HTMLButtonElement>('[data-act="pick-photo"]');
  const exportNoPhotoBtn = $<HTMLButtonElement>('[data-act="export-nophoto"]');
  const cancelPhotoBtn = $<HTMLButtonElement>('[data-act="cancel-photo"]');
  const archeSel = $<HTMLSelectElement>('[data-style="archetype"]');
  const fontSel = $<HTMLSelectElement>('[data-style="font"]');
  const densitySel = $<HTMLSelectElement>('[data-style="density"]');
  const shapeSel = $<HTMLSelectElement>('[data-style="photoShape"]');
  const fitEl = $<HTMLInputElement>('[data-style="fit"]');
  const addPhotoBtn = $<HTMLButtonElement>('[data-act="add-photo"]');
  const reviseBtn = $<HTMLButtonElement>('[data-act="revise"]');
  const tailorBtn = $<HTMLButtonElement>('[data-act="tailor"]');
  const jdPanel = $<HTMLElement>(".rx-jd");
  const jdInput = $<HTMLTextAreaElement>("[data-jd]");
  const jdRunBtn = $<HTMLButtonElement>('[data-act="jd-run"]');
  const jdCancelBtn = $<HTMLButtonElement>('[data-act="jd-cancel"]');
  const photoChipEl = $<HTMLElement>(".rx-photochip");

  let sessionId = "";
  let busy = false;
  let current: ResumeDraft | null = null;
  /** draft = 显示文字稿（能看到「待补充」缺口）；real = 显示真实版式预览 */
  let previewMode: "draft" | "real" = "draft";
  /** 当前生效的证件照（data URL） */
  ...
  const styleState = () => ({
    archetype: archeSel.value as "standard" | "skill-first" | "project-first",
    font: fontSel.value as "sans" | "serif",
    density: densitySel.value as "loose" | "normal" | "compact",
    photoShape: shapeSel.value as "square" | "round",
  });

  function setBusy(on: boolean, note = "") {
    busy = on;
    updateSendState();
  ...
  function updateSendState() {
    ...
    if (hasText && !busy) hintEl.textContent = "";
  }
  ...
  function pushMsg(role: "user" | "agent" | "err", text: string) {
  ...
  function renderChips() {
    if (chipsEl.dataset.done === "1") return;
    chipsEl.innerHTML = EXAMPLES.map(
      (t) => `<button class="rx-chip" type="button">${escapeHtml(t)}</button>`
    ).join("");
    chipsEl.querySelectorAll<HTMLButtonElement>(".rx-chip").forEach((btn) => {
  ...
  function renderDoc(markdown: string) {
    ...
    docEl.innerHTML = mdToHtml(markdown).replace(
      /（待补充）/g,
      '<span class="placeholder">（待补充）</span>'
    );
  }

  function renderDraft(draft: ResumeDraft) {
    current = draft;
    ...
    if (previewMode === "real") void showRealPreview();
    else renderDoc(draft.markdown);
    ...
    if (draft.missing.length > 0) {
      ...
      missingEl.innerHTML = `<h4>还缺这些信息（补得越多，简历越有竞争力）</h4><ul>${draft.missing
        .map(
          (m) =>
            `<li><span class="tag">${
              m.priority === "high" ? "重要" : m.priority === "medium" ? "建议" : "可选"
            }</span><span class="q">${escapeHtml(m.label)}</span><div class="why">${escapeHtml(
              m.why
            )}</div></li>`
        )
        .join("")}</ul>`;
  ...
  async function send(text: string) {
    ...
    try {
      if (!sessionId) {
        const draft = await deps.plugin.capability<{ raw: string }, ResumeDraft>("intake").run(
          { raw: content },
          deps.ctx
        );
        renderDraft(draft);
        pushMsg("agent", draft.reply);
        if (draft.nextQuestion) pushMsg("agent", `👉 ${draft.nextQuestion}`);
        ...
        const draft = await deps.plugin
          .capability<{ sessionId: string; answer: string }, ResumeDraft>("refine")
  ...
  async function compose() {
    ...
    try {
      const draft = await deps.plugin
        .capability<{ sessionId: string }, ResumeDraft>("compose")
  ...
  /** 读取图片文件，先做本地体检再交给回调 */
  function readImageFile(file: File, onOk: (dataUrl: string) => void) {
   ...
   */
  async function applyPhoto(dataUrl: string) {
    ...
    if (!sessionId) {
      ...
      pushMsg(
        "agent",
        "收到，照片已存好——不用再点发送。等我们开始整理简历，它会自动贴到右上角。"
      );
      return;
    }
    ...
    try {
      await deps.plugin
        .capability<{ sessionId: string; photo: string }, { hasPhoto: boolean }>("set-photo")
  ...
  async function clearPhoto() {
    ...
    try {
      await deps.plugin
        .capability<{ sessionId: string; photo: string }, unknown>("set-photo")
        .run({ sessionId, photo: "" }, deps.ctx);
      if (previewMode === "real") await showRealPreview();
    } catch {
  ...
  async function showRealPreview() {
    ...
    try {
      const out = await deps.plugin
        .capability<
          { sessionId: string; format: "html"; style: unknown },
          { html: string }
        >("export")
        .run({ sessionId, format: "html", style: styleState() }, deps.ctx);

      docEl.innerHTML = `<iframe sandbox="" title="简历版式预览"></iframe>`;
      const frame = docEl.querySelector("iframe") as HTMLIFrameElement | null;
  ...
  async function doExport(photo?: string) {
    if (!sessionId || busy) return;
    ...
    try {
      ...
      const out = await deps.plugin
        .capability<Record<string, unknown>, ExportOutput>("export")
        .run(input, deps.ctx);
      ...
      if (out.pdf) {
        ...
        pushMsg(
          "agent",
          `${out.pdfUnavailableReason ?? "已生成可打印页面"}。在新窗口里选择「另存为 PDF」即可保存。`
        );
      }
  ...
  const onRevise = () => {
    ...
    pushMsg(
      "agent",
      "想改哪里直接说——补充一段经历、调个措辞，或者指出哪句你觉得写得不好，我来改。"
    );
  };
  ...
  const onJdRun = async () => {
    ...
    if (jd.length < 30) {
      pushMsg(
        "err",
        "这段太短了，判断不出岗位要什么。把「岗位职责 + 任职要求」整段粘进来再试。"
      );
      jdInput.focus();
      return;
    }
    ...
    try {
      const out = await deps.plugin
        .capability<
          { sessionId: string; jd: string },
          { offTopic: boolean; markdown: string; reply?: string }
        >("tailor")
  ...
  const onStyleChange = () => {
    if (sessionId) void showRealPreview();
  };
