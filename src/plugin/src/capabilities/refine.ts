...
import { COMPOSE_THRESHOLD, mergeFacts, type ResumeDraft } from "../model.ts";
import { buildRefineMessages } from "../prompts/refine.ts";
import {
  MAX_INPUT,
  buildDraft,
  callLlmJson,
 ...
 */
export function createRefineCapability(host: HostServices): Capability<RefineInput, ResumeDraft> {
  return {
    id: "refine",
    async run(input: RefineInput, ctx: CallContext): Promise<ResumeDraft> {
      ...
      if (!answer) throw new PluginError("E_INPUT_INVALID", "回答为空");
      if (looksLikeOverride(answer)) throw new PluginError("E_INPUT_INVALID", "回答包含疑似指令内容");
