...
- 目标岗位/JD 里要求、但用户素材中完全没有依据的技能或经历：不得写成实战经验，只能二选一——①以「学习中（暂无实战）」的措辞出现在技能栏，或 ②干脆不写。任何情况下不得使用「接触过」「熟悉」「有经验」这类暗示有基础的表述。凡命中此条，必须在 reply 里明确告诉用户：「JD 要求了 X，你的简历还没有相关依据」，由用户决定补充真实经历还是保持现状。`;
