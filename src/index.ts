/**
 * DSH 插件入口：把简历专家的四阶段做成 6 个 Agent 可调用的工具。
 * 用户在 DSH 对话里说"帮我做份简历"，Agent 自己链式调用这些工具。
 */
...
import { resumeExpertPlugin } from "./plugin/src/index.ts";
import { buildHostServices, buildContext } from "./adapter.ts";

export const name = "resume-expert";

export const inject = ["tools"]; // 必需：等工具注册表就绪

// —— 各能力返回值的最小形状（只取工具输出需要的字段）——
...
type CapResult = DraftResult & ExportResult;
...
export function apply(ctx: Context) {
  ...
  const missingOf = (r: DraftResult) =>
    (r.missing ?? []).map((m) => m.label).join("、") || "无";

  ctx.tools.register(
    defineTool({
      name: "resume_intake",
      description:
        "简历专家·阶段一：用户说要做简历/改简历时首先调用。输入用户关于自己的原始描述，返回 sessionId、第一版文字稿与待补充清单。记住返回的 sessionId，后续工具都要用。",
      parameters: {
      ...
      async execute(args, exec) {
        const r = (await instance
          .capability("intake")
          .run({ raw: args.raw }, buildContext(exec.signal))) as CapResult;
        return [
  ...
  ctx.tools.register(
    defineTool({
      name: "resume_refine",
      description:
        "简历专家·阶段二：把用户补充的信息（姓名/教育/实习/项目/技能等）合并进指定会话。可多轮调用。",
      parameters: {
      ...
      async execute(args, exec) {
        const r = (await instance
          .capability("refine")
          .run({ sessionId: args.sessionId, answer: args.answer }, buildContext(exec.signal))) as CapResult;
        return [
          `完整度: ${r.completeness}%`,
          `待补充: ${missingOf(r)}`,
          `追问: ${r.nextQuestion}`,
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        const r = (await instance
          .capability("compose")
          .run({ sessionId: args.sessionId }, buildContext(exec.signal))) as CapResult;
  ...
  ctx.tools.register(
    defineTool({
      name: "resume_export",
      description:
        "简历专家·阶段四：把定稿渲染成 A4 排版并导出 PDF（本机无 Chrome 时导出可打印 HTML）。必须先 resume_compose。",
      parameters: {
        sessionId: { type: "string", required: true },
      ...
      async execute(args, exec) {
        const r = (await instance
          .capability("export")
          .run(
            { sessionId: args.sessionId, fitSinglePage: args.fitSinglePage === true },
            buildContext(exec.signal),
          )) as CapResult;
        const ex = r as ExportResult;
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        "简历专家·证件照：把本机一张照片（png/jpg/webp，≤3MB）设为指定会话的证件照，导出时出现在右上角。",
      parameters: {
        sessionId: { type: "string", required: true },
      ...
      async execute(args, exec) {
        ...
        const r = (await instance
          ...
          .run({ sessionId: args.sessionId, photo }, buildContext(exec.signal))) as CapResult;
  ...
  ctx.tools.register(
    defineTool({
      name: "resume_tailor",
      description:
        "简历专家·按 JD 定制：把指定会话的简历按目标岗位 JD 重写（会替换当前版本，导出前建议先 resume_export 留底）。",
      parameters: {
        sessionId: { type: "string", required: true },
      ...
      async execute(args, exec) {
        const r = (await instance
          .capability("tailor")
          .run({ sessionId: args.sessionId, jd: args.jd }, buildContext(exec.signal))) as CapResult;
